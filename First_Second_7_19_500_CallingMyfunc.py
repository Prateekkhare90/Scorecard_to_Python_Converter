...
# #         print(p)
    ...
    Mergewords2.insert(0, "import pandas as pd")
    Mergewords2.insert(1, "def myfunc(DF):")
    #Mergewords2.insert(2, "    DF=pd.read_csv('Test_data.csv')")
#     Mergewords2.insert(0, "def if_elif_statements(DF):")
    Mergewords2.insert(len(Mergewords2), "    return DF" )
    print(Mergewords2)
    
    log = open("Py_elif48.py", "w")
...
def Scorecard_to_python2(q):
    ...
    DF2['Score_total'] = DF2[DF2.columns[-4:]].sum(axis=1)
    display(DF2)
    DF2.to_csv('Out_DF52.csv')
...
get_ipython().system('jupyter nbconvert --to script First_Second_7_19_500_CallingMyfunc.ipynb')


# In[ ]:

get_ipython().system('python First_Second_7_19_500_CallingMyfunc.py SASEM_Output.csv Test_data.csv')
